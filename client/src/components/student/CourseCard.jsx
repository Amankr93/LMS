import { Currency, Star } from 'lucide-react'
import React, { useContext } from 'react'
import { assets } from '../../assets/assets'
import AppContext from '../../context/AppContext'
import { Link } from 'react-router-dom'


const CourseCard = ({ course }) => {
  const { calculateRating, currency } = useContext(AppContext)
  const rating = calculateRating(course);

  return (
    <Link onClick={()=>scrollTo(0,0)} className='border border-gray-500/30 pb-6 overflow-hidden rounded-lg' to={'/course/'+course._id}>
      <img src={course.courseThumbnail} alt="" className='w-full' />
      <div className='p-3 text-left'>
        <h3 className='text-base font-semibold'>{course.courseTitle}</h3>
        <p className='text-gray-500'>{course.educator.name}</p>
        <div className='flex items-center space-x-2'>
          <p>{rating}</p>
          <div className='flex '>
            {
              [...Array(5)].map((_, i) => {
                return i < Math.floor(rating) ? <img key={i} src={assets.star} alt="" className='w-3.5 h-3.5' /> :
                  <img key={i} src={assets.star_blank} alt="" className='w-3.5 h-3.5' />

              })
            }
          </div>
          <p className='text-gray-500'>22</p>
        </div>
        <p>{currency}{(course.coursePrice - course.coursePrice * course.discount / 100).toFixed(2)}</p>

      </div>

    </Link>
  )
}

export default CourseCard
