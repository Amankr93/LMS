...
const AddCourse = () => {
  ...
  return (
    <div className='h-screen overflow-scroll flex felx-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form action="" className='flex flex-col gap-4 max-w-full text-gray-500' onSubmit={(e)=>handleSubmit(e)}>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input type="text" onChange={e => setCourseTitle(e.target.value)} value={courseTitle} placeholder='Type Here' className='outline-none px-3 py-2 rounded border border-gray-500' />
        </div>
        <div className='flex flex-col gap-1'>
        ...
        <div className='flex justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            <p>Course Price</p>
            <input type="number" onChange={e => setCoursePrice(e.target.value)} value={coursePrice} className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required />
          </div>
        ...
        <div>
                  ...
                  !chapter.collapsed && <div className='p-4'>
                    {chapter.chapterContent.map((lecture, lectureIndex) => {
                      return <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                        <span>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins - <a href={lecture.lectureUrl} className='text-blue-500' target='blank'>Link</a> - {lecture.isPreviewFree ? 'Free Preview' : 'Paid'}</span>
          ...
          <div className='flex justify-center items-center bg-blue-100 p-2 rounded-lg cursor-pointer' onClick={e => handleChapter('add')}>+ Add Chapter</div>
          {showPopup && (
            <div className='fixed inset-0 flex justify-center items-center bg-gray-800/50'>
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80 '>
                ...
                <div className='mb-2'>
                  <p>Lecture Title</p>
                  <input type="text"
                    className='mt-1 block border rounded py-1 px-2'
                ...
                <button type='button' className='w-full bg-blue-400 text-white px-4 py-2 rouded ' onClick={addLecture}>Add</button>
                <img src={assets.cross_icon} className='absolute top-4 right-4 w-4 cursor-pointer' alt="" onClick={() => setShowPopup(false)} />
