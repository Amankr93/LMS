...
const Player = () => {
  const { enrolledCourses, navigate, calculateRating, calculateTotalLecture, calculateDuration } = useContext(AppContext)
  const { courseId } = useParams();
  const [course, setCourse] = useState(null)
  const [openSection, setOpenSection] = useState({})
  const [playerData, setPlayerData] = useState(null)
  useEffect(() => {
    setCourse(enrolledCourses.find((course) => course._id === courseId))
  }, [enrolledCourses])
                      ...
                      chapter.chapterContent.map((lecture, i) => {
                        return (
                          <div className='flex justify-between items-center px-5 py-2 ' key={i}>
                            <h3 className='flex gap-3 items-center text-gray-700 font-medium text-[15px]'><img src={assets.play_icon} alt="play icon" /> {lecture.lectureTitle}</h3>
                            <div className='flex gap-2'>
                              {
                                lecture.lectureUrl && <p className='text-sm text-blue-500 cursor-pointer' onClick={()=>setPlayerData({...lecture, chapter:index+1, lecture:i+1})}>Watch</p>
                              }
              ...
              <div className='shadow-(--custom-card) rounded-t overflow-hidden md:rounded-none bg-white  mt-4 rounded-md py-4'>
                <YouTube videoId={playerData.lectureUrl.split('/').pop()}  opts={{playerVars: {autoplay:1}}} iframeClassName='w-full aspect-video'/>
                 ...
                 <div className='flex  items-center w-full justify-between mt-4 px-4'>
                  ...
                  <button className='text-white bg-blue-600 px-3 py-2 rounded'>{false? 'Completed' :"Mark Completed"}</button>
                 </div>
              </div>
            ): 
